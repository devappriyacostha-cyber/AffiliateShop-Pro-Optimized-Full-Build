    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col bg-slate-950 p-6 text-white lg:flex">
        ...
        <nav className="flex-1 space-y-1.5">
          {tabs.map(([id, Icon, label]) => (
            <button key={id} onClick={() => setActiveTab(id)} className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3.5 text-left text-xs font-black uppercase tracking-wider transition ${activeTab===id ? "bg-orange-600 text-white shadow-lg shadow-orange-950/30" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}>
              <Icon size={17}/> {label}
            </button>
          ))}
        </nav>
      ...
      <main className="min-h-screen lg:pl-72">
        ...
        <div className="p-5 lg:p-10">
          ...
          {activeTab === "analytics" && (
            <div className="space-y-6"><div className="grid gap-5 md:grid-cols-3">{[["All time",analytics.totalClicks],["Today",analytics.todayClicks],["Last 7 days",analytics.last7Clicks]].map(([label,value])=><div key={label} className="rounded-[28px] border bg-white p-7 shadow-sm"><div className="text-[10px] font-black uppercase tracking-widest text-slate-400">{label}</div><div className="mt-3 text-4xl font-black">{Number(value).toLocaleString()}</div></div>)}</div><div className="rounded-[28px] border bg-white shadow-sm"><div className="flex items-center justify-between border-b p-6"><h2 className="font-black">Analytics controls</h2><div className="flex flex-wrap gap-2">{(["today","7days","30days","all"] as const).map(p=><button key={p} onClick={()=>resetAnalytics(p)} className="rounded-xl border px-3 py-2 text-[10px] font-black uppercase hover:border-red-400 hover:text-red-600">Clear {p}</button>)}</div></div><div className="divide-y">{products.slice().sort((a,b)=>(b.clicks||0)-(a.clicks||0)).map(p=><div key={p.id} className="flex justify-between p-5"><span className="font-bold">{p.shortName||p.name}</span><span className="font-black text-orange-600">{p.clicks||0} clicks</span></div>)}</div></div></div>
          )}
